...
- **v2 API direct requests**: Client-side requests connect directly to `NEXT_PUBLIC_BACKEND_API_URL/v2/*` (defaults to `https://api.proxybase.xyz/v2/*`), eliminating Vercel compute usage. Backend CORS handles cross-origin requests. SSE auth uses `?token=` (EventSource can't set headers).
...
- **Console UI**: `app/console/` (page, layout, 3 tabs — market / deposits / wallet; TelemetryTab.js exists but is not linked in the nav), state in `app/lib/AuthContext.js` (Google session → wallet resolution → v2 challenge/verify handshake → `sk_` token in localStorage). Styling follows `console_design.md` (Coinbase-style system): white canvas + single `#0052ff` accent, pill CTAs, hairline borders, Inter + JetBrains Mono, implemented as `--pb-*` CSS tokens in `app/globals.css`. Light theme is default; dark theme via `[data-console-theme="dark"]` on `<html>`, toggled from the console header and persisted as `pb-console-theme` in localStorage (state lives in AuthContext).
